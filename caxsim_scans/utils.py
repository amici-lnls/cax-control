...
from shadowpy.optical_elements import Screen
...
def get_image(dvf: Screen):
    """Get image from Screen with retries on failure."""
    count = 0
    while count < Cfg.MAXERRORCOUNT:
        try:
            ...
            print(f" WARNING. When trying to fetch image from Screen1: {err} ")
            time.sleep(2)
            count += 1
...
def snapshot_dvf(dvf, *, include_imgproc=False):
    """Capture a Screen snapshot: image, camera settings, and beam diagnostics.

    Args:
        dvf: a Screen (or ScreenImgProc / CAXDtc) device instance.
        include_imgproc: if True, read beam-fit PVs (centroid, widths,
            tilt angle, ellipse sigmas).  Only meaningful for ScreenImgProc
            subclasses.
    ...
    if include_imgproc:
        snap.update({
            ...
            'roix_fit_mean'  : dvf.image.hprm_fitting['mux'],
            'roiy_fit_mean'  : dvf.image.hprm_fitting['muy'],

            # --- beam widths (1-D Gaussian fit sigmas, in pixels) ---
            'roix_fit_sigma' : dvf.image.hprm_fitting['sigx'],
            'roiy_fit_sigma' : dvf.image.hprm_fitting['sigy'],

            # --- FWHM of the ROI projections (pixels) ---
            'roix_fwhm'      : dvf.image.hprm_fitting['fwhmx'],
            'roiy_fwhm'      : dvf.image.hprm_fitting['fwhmy'],

            # --- 2-D ellipse fit (from second-moment SVD) ---
            'fit_angle'      : dvf.image.hprm_fitting['theta'],     # tilt angle [deg]
            'fit_sigma1'     : dvf.image.hprm_momenta['sigx'],    # major-axis sigma [px]
            'fit_sigma2'     : dvf.image.hprm_momenta['sigy'],    # minor-axis sigma [px]
...
def snapshot_machine_state(cax: CAXSim):
    """Get current beamline configuration as a dictionary."""
    caxm  = cax.mirror
    # Placeholder low limit and high limit values
    mirror_status = {
            'tx'    : [caxm.tx,
                        -5, 
                        5,
    ...
    # Screen1 status accounts for caustic scans.
    ...
    # Screen2 status accounts for lens and caustic scans.
    dvf_b1  = cax.dvf_B1
    dvf_b1_status = {
        # 'z_pos'    : [dvf_b1.shadow_oe,
        #               dvf_b1.z_lolm,
        #               dvf_b1.z_hilm,
...
# --- Screen sub-keys that hold images (saved as datasets, not attributes) ---
_Screen_IMAGE_KEY = 'image'
...
def save_step(h5file, step, step_index=None):
    ...
    HDF5 layout produced for each step::
        ...
        /scan-NNNN                          ← group  (one per step)
            ...
            dvf1           (dataset)        ← gzip-compressed image array
                @exposure_time    = 0.01    ← dataset attrs: Screen scalars
    ...
    # --- Separate Screen dicts (contain images) from everything else ---
    non_dvf = {k: v for k, v in step.items() if k not in DVF_KEYS}
    grp_attrs = _flatten_dict(non_dvf)

    h5file.save_group(grpname=grpname, grpmetadata=grp_attrs)

    # --- Save each Screen as a dataset inside the group ---
    for dvf_key in DVF_KEYS:
        dvf_dict = step.get(dvf_key)
        if dvf_dict is None:
            continue

        image = dvf_dict[_Screen_IMAGE_KEY]
        dset_attrs = _flatten_dict(
            {k: v for k, v in dvf_dict.items() if k != _Screen_IMAGE_KEY}
        )
        h5file.save_dataset(
            grpname=grpname,
            dsetname=dvf_key,
...
def image_show_slit_boundary(img):
    ...
    fig, ax = plt.subplots()
    pix_to_um = Cfg.SCALE
    print("\n ** Current slit boundaries at Screen (in microns):\n"
